...
// Propósito: (Presupuesto) CRUD para gestionar la ficha del personal.
...
// Comentario: Acción (listar, crear, editar). La eliminación podría ser marcar como 'inactivo' o si la ficha está ligada a un usuario, se maneja con el usuario.
$accion_ficha_crud = $_GET['accion_ficha_crud'] ?? 'listar';
$id_ficha_editar = null;
$ficha_para_editar = null;
$id_usuario_asociado_ficha = null; // Comentario: Para el select de usuario al crear/editar.
...
try {
    ...
    // Comentario: No es crítico para listar, pero sí para crear.
}
...
if ($accion_ficha_crud === 'editar' && isset($_GET['id_ficha'])) {
    ...
    if ($id_ficha_editar) {
        try {
            $stmt_edit_ficha = $pdo->prepare("SELECT pf.*, u.nombre_usuario
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['guardar_ficha_nueva']) || isset($_POST['actualizar_ficha_existente']))) {
    ...
    $fecha_nacimiento_form = sanitizar_entrada($_POST['fecha_nacimiento'] ?? ''); // Comentario: Validar formato YYYY-MM-DD
    $ci_numero_form = sanitizar_entrada($_POST['ci_numero'] ?? '');
    // Comentario: ... obtener todos los demás campos del formulario ...
    ...
    $salario_base_form = filter_var($_POST['salario_base'] ?? null, FILTER_VALIDATE_FLOAT, ['options' => ['min_range' => 0]]);
    ...
    $errores_form_ficha = [];
    if (empty($id_usuario_form)) $errores_form_ficha[] = "Debe seleccionar un usuario para asociar la ficha.";
    ...
    // Comentario: Verificar unicidad de id_usuario, codigo_empleado, ci_numero, nua_cua (excepto para la propia ficha al editar).
    $id_excluir_ficha_check = $id_ficha_form ?: 0;
    try {
        $checks = [
            'id_usuario' => [$id_usuario_form, "El usuario seleccionado ya tiene una ficha de personal."],
            ...
            'nua_cua' => [$nua_cua_form, "El NUA/CUA '$nua_cua_form' ya está registrado."]
        ];
        foreach ($checks as $campo_check => $data_check) {
            list($valor_check, $msj_error_check) = $data_check;
            if (!empty($valor_check)) { // Comentario: Solo chequear si el valor no está vacío.
    ...
    if (empty($errores_form_ficha)) {
        $params_sql_ficha = [
            ':id_usr' => $id_usuario_form, ':cod_emp' => $codigo_empleado_form,
            ...
            ':salario' => ($salario_base_form === false || is_null($salario_base_form)) ? null : $salario_base_form,
        ...
        try {
            if (isset($_POST['guardar_ficha_nueva'])) {
                $sql_insert_ficha = "INSERT INTO personal_fichas (id_usuario, codigo_empleado, fecha_nacimiento, lugar_nacimiento, nacionalidad, ci_numero, ci_expedido_en, estado_civil, domicilio_actual, telefono_emergencia, contacto_emergencia_nombre, relacion_contacto_emergencia, nivel_educativo, profesion, fecha_ingreso_institucion, tipo_contrato, salario_base, afp_asociada, nua_cua, grupo_sanguineo, alergias_conocidas, observaciones_medicas)
                                     VALUES (:id_usr, :cod_emp, :fec_nac, :lugar_nac, :nac, :ci_num, :ci_exp, :est_civil, :domicilio, :tel_emerg, :contacto_emerg_nom, :rel_contacto_emerg, :nivel_edu, :profesion, :fec_ingreso, :tipo_cont, :salario, :afp, :nua, :grupo_sang, :alergias, :obs_med)";
                ...
                $sql_update_ficha = "UPDATE personal_fichas SET
                                        id_usuario = :id_usr, codigo_empleado = :cod_emp, fecha_nacimiento = :fec_nac, lugar_nacimiento = :lugar_nac, nacionalidad = :nac,
        ...
        if (isset($_POST['actualizar_ficha_existente'])) {
            $accion_ficha_crud = 'editar'; // Comentario: Para repintar el form de edición con errores.
            $id_usuario_asociado_ficha = $id_usuario_form; // Comentario: Mantener el usuario.
            // Comentario: $ficha_para_editar debería recargarse o usar datos del POST.
        }
    }
}


// Comentario: Lógica para listar fichas.
$fichas_personal = [];
...
<?php if ($accion_ficha_crud === 'crear' || ($accion_ficha_crud === 'editar' && $ficha_para_editar)): ?>
    <h3><?php echo ($accion_ficha_crud === 'crear') ? 'Crear Nueva Ficha de Personal' : 'Editar Ficha de: ' . htmlspecialchars($ficha_para_editar['nombre_usuario'] ?? '', ENT_QUOTES, 'UTF-8'); ?></h3>
    <div class="card-sigi">
        <form action="index.php?vista=personal_ficha_crud" method="POST" class="validar-js form-ficha-personal">
            <?php if ($accion_ficha_crud === 'editar'): ?>
            ...
            <fieldset>
                ...
                <div class="grupo-formulario">
                    ...
                    <?php if ($accion_ficha_crud === 'editar' && $id_usuario_asociado_ficha): ?>
                        ...
                        <p><strong><?php echo htmlspecialchars($ficha_para_editar['apellidos'] . ', ' . $ficha_para_editar['nombres'] . ' (' . $ficha_para_editar['nombre_usuario'] . ')', ENT_QUOTES, 'UTF-8'); ?></strong> (No editable desde aquí)</p>
                    <?php else: ?>
                        <select id="id_usuario_ficha" name="id_usuario_ficha" required>
                            <option value="">-- Seleccione un usuario --</option>
                            <?php foreach ($usuarios_sin_ficha as $usf): ?>
                                <option value="<?php echo $usf['id_usuario']; ?>" <?php if(($ficha_para_editar['id_usuario'] ?? ($_POST['id_usuario_ficha'] ?? '')) == $usf['id_usuario']) echo 'selected'; ?>>
            ...
            <fieldset>
                ...
                <div class="grid-col-2">
                    ...
                    <div class="grupo-formulario">
                        ...
                        <input type="number" id="salario_base" name="salario_base" step="0.01" value="<?php echo htmlspecialchars($ficha_para_editar['salario_base'] ?? ($_POST['salario_base'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
                    </div>
                </div>
            </fieldset>
...
<?php if ($accion_ficha_crud === 'listar'): ?>
                    ...
                    <?php foreach ($fichas_personal as $ficha_item): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <!-- Comentario: La eliminación de fichas podría ser compleja si está ligada a usuarios activos. -->
                                <!-- Comentario: Considerar desactivar al usuario o la ficha en lugar de borrar. -->
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
...
.form-ficha-personal fieldset legend { font-size: 1.1em; font-weight: bold; padding: 0 0.5em; width: auto; } /* Comentario: Ajustar ancho automático para legend. */
